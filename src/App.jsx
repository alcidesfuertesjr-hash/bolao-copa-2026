...
function Flag({ cc, nome }) {
  ...
  if (!emoji) return <span title={nome} style={{ fontSize: 18 }}>🏳️</span>;
  return <span title={nome} style={{ fontSize: 22, lineHeight: 1, verticalAlign: 'middle' }}>{emoji}</span>;
}
...
export default function BolaoApp() {
  ...
  // pick first usuario by default
  useEffect(() => {
    if (state && !usuario && state.usuarios.length) setUsuario(state.usuarios[0]);
  }, [state, usuario]);
    ...
    <div style={page}>
      <div style={container}>
        ...
        {tab === 'jogos' && (
          <>
            <div style={card}>
              <div style={h3}>Seu usuário</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                <select style={select} value={usuario} onChange={e => setUsuario(e.target.value)}>
                  {state.usuarios.map(u => <option key={u} value={u}>{u}</option>)}
                </select>
                <input style={textInput} placeholder="Novo nome" value={novoUsuario} onChange={e => setNovoUsuario(e.target.value)} />
                <PrimaryButton small onClick={addUsuario}>Adicionar</PrimaryButton>
              </div>
            </div>
