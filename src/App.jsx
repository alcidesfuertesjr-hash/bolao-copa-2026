...
function SplashScreen({ onDone }) {
  ...
  const [isBrasil, setIsBrasil] = React.useState(false);
  ...
  function playSound(sequencia) {
    try {
      ...
      cg.gain.setValueAtTime(0, tFinal);
      cg.gain.linearRampToValueAtTime(0.5, tFinal + 0.7);
      cg.gain.linearRampToValueAtTime(0, tFinal + 2.5);
      src.start(tFinal);
      const w = ctx.createOscillator(), wg = ctx.createGain();
      w.connect(wg); wg.connect(ctx.destination); w.type = 'sine';
      w.frequency.setValueAtTime(2800, tFinal + 0.2);
      w.frequency.linearRampToValueAtTime(3400, tFinal + 0.7);
      wg.gain.setValueAtTime(0.3, tFinal + 0.2);
      wg.gain.exponentialRampToValueAtTime(0.001, tFinal + 1);
  ...
  const ringSize = 'min(370px, 80vw)';
  const slotSize = 'min(65px, 14vw)';
  const colors = ['#FFD700','#009C3B','#FFFFFF','#002776','#F9A021','#FD3F43'];
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes brasilPulse{0%,100%{box-shadow:0 0 0 4px #FFD700,0 0 0 7px #009C3B,0 0 50px rgba(0,156,59,0.8),0 0 80px rgba(255,215,0,0.6)}50%{box-shadow:0 0 0 4px #FFD700,0 0 0 10px #009C3B,0 0 70px rgba(0,156,59,1),0 0 120px rgba(255,215,0,0.8)}}
        @keyframes centerBrasil{0%,100%{transform:scale(1.15)}50%{transform:scale(1.25)}}
      ...
      {/* confetes quando Brasil */}
      {isBrasil && colors.map((color, ci) =>
      ...
      <div style={{
        ...
        animation: ringStop ? 'none' : 'ringRotate 18s linear infinite',
        flexShrink: 0,
      }}>
        {times.map((tm, i) => {
          const angle = (i / N) * 360;
          ...
          const isHighlight = i === current;
          const isBrasilSlot = isBrasil && i === BRASIL_IDX;

          return (
            <div key={i} style={{
              position: 'absolute', top: '50%', left: '50%',
              width: slotSize, height: slotSize,
              marginTop: 'calc(min(65px,14vw) / -2)',
              marginLeft: 'calc(min(65px,14vw) / -2)',
              transform: `translate(${x}px, ${y}px)`,
            }}>
              <div style={{
                width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden',
                position: 'relative',
                border: isBrasilSlot ? '3px solid #FFD700' : isHighlight ? '3px solid #FFD700' : '3px solid rgba(255,255,255,0.2)',
                boxShadow: isBrasilSlot
                  ? '0 0 0 3px #009C3B,0 0 50px rgba(0,156,59,0.8),0 0 80px rgba(255,215,0,0.6)'
                  : isHighlight ? '0 0 0 3px #FFD700,0 0 25px rgba(255,215,0,0.8)' : '0 4px 15px rgba(0,0,0,0.4)',
                transform: isHighlight ? 'scale(1.45)' : 'scale(1)',
                transition: 'all 0.2s ease',
                animation: isBrasilSlot ? 'brasilPulse 0.8s ease-in-out infinite' : 'none',
                zIndex: isHighlight ? 20 : 1,
              }}>
      ...
      <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', textAlign:'center', zIndex:5, pointerEvents:'none' }}>
        <div style={{
          width:'min(100px,22vw)', height:'min(100px,22vw)', borderRadius:'50%', overflow:'hidden',
          border: isBrasil ? '4px solid #FFD700' : '4px solid #FFD700',
          boxShadow: isBrasil ? '0 0 0 4px #009C3B,0 0 60px rgba(0,156,59,0.9),0 0 100px rgba(255,215,0,0.6)' : '0 0 0 3px rgba(255,215,0,0.4),0 0 40px rgba(255,215,0,0.5)',
          margin:'0 auto 8px',
          animation: isBrasil ? 'centerBrasil 1s ease-in-out infinite' : 'none',
          transition: 'box-shadow 0.3s',
        }}>
          <img src={flagUrl(t.cc)} alt={t.nome} style={{ width:'140%', height:'140%', objectFit:'cover', marginLeft:'-20%', marginTop:'-20%' }} />
        </div>
        <div style={{
          fontSize: isBrasil ? 'clamp(15px,3.5vw,26px)' : 'clamp(12px,2.5vw,19px)',
          color:'#FFD700',
          textShadow: isBrasil ? '0 0 30px rgba(0,156,59,0.9),0 0 60px rgba(255,215,0,0.8)' : '0 0 20px rgba(255,215,0,0.8)',
      ...
      {showPergunta && (
        <div style={{ position:'absolute', bottom:'8%', textAlign:'center', zIndex:10, animation:'perguntaIn 0.8s ease-out forwards' }}>
          <p style={{ fontSize:'clamp(15px,3vw,26px)', color:'#fff', letterSpacing:'0.06em', textShadow:'0 2px 8px rgba(0,0,0,0.5)' }}>Quem será o</p>
          <span style={{ color:'#FFD700', fontSize:'clamp(19px,4vw,34px)', textShadow:'0 0 30px rgba(255,215,0,0.8)', display:'block', marginTop:5 }}>⚽ CAMPEÃO? ⚽</span>
