      ...
      <div style={{ ...container, position: 'relative', zIndex: 1 }}>
        ...
        {effectiveTab === 'rank' && (
          <div style={card}>
                ...
                <tbody>
                  {ranking.map((r, i) => (
                    <tr
                      ...
                      onClick={() => setRankExpanded(rankExpanded === r.usuario ? null : r.usuario)}
                    >
                      ...
                      <td style={{ padding: '10px 10px', fontWeight: 700 }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontSize: 10, color: '#94A3B8' }}>{rankExpanded === r.usuario ? '▲' : '▼'}</span>
                          {r.usuario}
                        </span>
                      </td>
            ...
            {rankExpanded && (() => {
              const r = ranking.find(x => x.usuario === rankExpanded);
              if (!r) return null;
              const jogosEncerrados = state.jogos
                .filter(j => j.placarOficialA !== null && j.placarOficialA !== undefined)
                .sort((a, b) => new Date(a.dataJogo) - new Date(b.dataJogo));
              return (
                <div style={{ marginTop: 16, borderTop: '2px solid #EEF2FF', paddingTop: 14 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#0E2080', marginBottom: 10 }}>
                    📋 Detalhamento de <span style={{ color: '#1530A6' }}>{rankExpanded}</span> — {jogosEncerrados.length} jogos encerrados
                  </div>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                      <thead>
                        <tr style={{ background: '#EEF2FF' }}>
                          <th style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Jogo</th>
                          <th style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Palpite</th>
                          <th style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Resultado</th>
                          <th style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Pts</th>
                          <th style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Situação</th>
                        </tr>
                      </thead>
                      <tbody>
                        {jogosEncerrados.map(jogo => {
                          const palpite = state.palpites[`${jogo.id}_${rankExpanded}`];
                          const pts = calcPontosJogo(palpite, jogo);
                          const faseInfo = FASES[jogo.fase] || FASES.grupos;
                          let situacao = '—', corPts = '#94A3B8', bgRow = 'transparent';
                          if (!palpite) { situacao = 'Sem palpite'; corPts = '#CBD5E1'; }
                          else if (pts === faseInfo.exato) { situacao = '🎯 Placar exato!'; corPts = '#16A34A'; bgRow = '#F0FDF4'; }
                          else if (pts === faseInfo.vencedor) { situacao = '✅ Acertou vencedor'; corPts = '#2563EB'; bgRow = '#EFF6FF'; }
                          else if (pts === 0) { situacao = '❌ Errou'; corPts = '#DC2626'; bgRow = '#FFF5F5'; }
                          return (
                            <tr key={jogo.id} style={{ borderTop: '1px solid #F1F5F9', background: bgRow }}>
                              <td style={{ padding: '5px 8px', fontWeight: 600 }}>
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexWrap: 'nowrap' }}>
                                  <Flag cc={jogo.ccA} nome={jogo.timeA} />
                                  <span style={{ whiteSpace: 'nowrap' }}>{jogo.timeA}</span>
                                  <span style={{ color: '#94A3B8' }}>x</span>
                                  <span style={{ whiteSpace: 'nowrap' }}>{jogo.timeB}</span>
                                  <Flag cc={jogo.ccB} nome={jogo.timeB} />
                                </span>
                              </td>
                              ...
                              <td style={{ padding: '5px 8px', color: corPts, fontWeight: 600, whiteSpace: 'nowrap' }}>
                                {situacao}
                              </td>
                            </tr>
                          );
                      ...
                      <tfoot>
                        <tr style={{ borderTop: '2px solid #E2E8F0', background: '#EEF2FF' }}>
                          <td colSpan={3} style={{ padding: '8px 8px', fontWeight: 700, color: '#0E2080' }}>Total em jogos</td>
