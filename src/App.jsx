...
export default function BolaoApp() {
  ...
  useEffect(() => {
    ...
    const atualizacoes = calcularClassificadosR32(state.jogos);
    if (Object.keys(atualizacoes).length === 0) return;

    let mudou = false;
    const novosJogos = state.jogos.map(j => {
      const upd = atualizacoes[j.id];
      if (!upd) return j;
      const novoA = upd.nomeA || j.timeA;
      const novoB = upd.nomeB || j.timeB;
