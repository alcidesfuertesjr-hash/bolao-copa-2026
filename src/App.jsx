...
function SplashScreen({ onDone }) {
  const [phase, setPhase] = React.useState(0); // 0=dark, 1=flag, 2=texts, 3=confetti
  ...
  function playSound() {
    try {
      ...
      cg.gain.setValueAtTime(0, ctx.currentTime + 0.8);
      cg.gain.linearRampToValueAtTime(0.5, ctx.currentTime + 1.8);
      cg.gain.linearRampToValueAtTime(0, ctx.currentTime + 4.5);
      ...
      w.frequency.setValueAtTime(2800, ctx.currentTime + 1.5);
      w.frequency.linearRampToValueAtTime(3600, ctx.currentTime + 2.0);
      wg.gain.setValueAtTime(0.3, ctx.currentTime + 1.5);
      wg.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 2.2);
      w.start(ctx.currentTime + 1.5); w.stop(ctx.currentTime + 2.2);
    } catch(e) {}
  }
  ...
  const confettiColors = ['#C60B1E','#F1BF00','#FFFFFF','#FF6B35','#FFD700','#AA0011'];

  return (
    <div style={{
      position:'fixed', inset:0, zIndex:999, overflow:'hidden',
      display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
      opacity: hiding ? 0 : 1, transition:'opacity 0.7s',
      fontFamily:"'Impact','Arial Black',system-ui,sans-serif",
    }}>
      <style>{`
        @keyframes trophyFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-12px)}}
        @keyframes flagGlow{0%,100%{box-shadow:0 0 0 4px #FFD700,0 0 60px rgba(255,215,0,0.7),0 12px 50px rgba(0,0,0,0.6)}50%{box-shadow:0 0 0 5px #FFD700,0 0 100px rgba(255,215,0,1),0 12px 50px rgba(0,0,0,0.6)}}
        @keyframes starPulse{0%,100%{filter:drop-shadow(0 0 8px rgba(255,215,0,0.9))}50%{filter:drop-shadow(0 0 20px rgba(255,215,0,1))}}
        @keyframes cfFall{0%{opacity:1;transform:translateY(0) rotate(0deg)}100%{opacity:0;transform:translateY(110vh) rotate(720deg)}}
        @keyframes ll{0%,100%{transform:rotate(8deg)}50%{transform:rotate(22deg)}}
        @keyframes lr{0%,100%{transform:rotate(-8deg)}50%{transform:rotate(-22deg)}}
      `}</style>

      {/* fundo escuro */}
      <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse at 50% 40%, #1530A6 0%, #0A1A5C 70%)', transition:'opacity 0.8s', opacity: phase >= 1 ? 0 : 1 }} />

      {/* fundo vermelho espanha */}
      <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse at 50% 40%, #C60B1E 0%, #800010 60%, #0A0A0A 100%)', transition:'opacity 1s', opacity: phase >= 1 ? 1 : 0 }} />

      {/* holofotes */}
      <div style={{position:'absolute',top:0,left:'12%',width:3,height:'65vh',background:'linear-gradient(to bottom,rgba(255,220,100,0.4),transparent)',transformOrigin:'top center',animation:'ll 4s ease-in-out infinite'}}/>
      <div style={{position:'absolute',top:0,right:'12%',width:3,height:'65vh',background:'linear-gradient(to bottom,rgba(255,220,100,0.4),transparent)',transformOrigin:'top center',animation:'lr 4s ease-in-out infinite'}}/>

      {/* confetes */}
      {phase >= 3 && confettiColors.map((color, ci) =>
        Array.from({length:10}).map((_, i) => (
          <div key={`${ci}-${i}`} style={{
            ...
            animation:`cfFall ${2+Math.random()*3}s ${Math.random()*2}s linear infinite`,
          }}/>
        ))
      )}
      ...
      <div style={{
        ...
        marginBottom:4, position:'relative', zIndex:10,
      }}>🏆</div>

      {/* estrelas acima da bandeira */}
      <div style={{
        textAlign:'center', fontSize:'clamp(28px,7vw,48px)',
      ...
      <div style={{
        ...
        <div style={{
          width:'min(300px,65vw)', height:'min(200px,43vw)',
          ...
          <svg viewBox="0 0 3 2" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',height:'100%'}}>
            ...
            <rect x="1.15" y="0.65" width="0.15" height="0.55" fill="#c60b1e"/>
            <rect x="1.7" y="0.65" width="0.15" height="0.55" fill="#c60b1e"/>
            <rect x="1.15" y="0.65" width="0.7" height="0.08" fill="#c60b1e"/>
            <rect x="1.15" y="1.12" width="0.7" height="0.08" fill="#c60b1e"/>
            <rect x="1.28" y="0.73" width="0.44" height="0.39" fill="#f1bf00" opacity="0.3"/>
          </svg>
        </div>
      </div>
      ...
      {/* textos */}
      <div style={{
        fontSize:'clamp(13px,3vw,22px)', color:'rgba(255,255,255,0.85)',
        letterSpacing:'0.25em', textTransform:'uppercase',
        fontFamily:'system-ui', fontWeight:700, textAlign:'center',
        opacity: phase >= 2 ? 1 : 0, transform: phase >= 2 ? 'translateY(0)' : 'translateY(15px)',
        transition:'all 0.5s ease-out', position:'relative', zIndex:10,
      }}>🏆 Campeã do Mundo 🏆</div>

      <div style={{
        fontSize:'clamp(32px,8vw,64px)', color:'#FFD700',
        letterSpacing:'0.08em',
        textShadow:'0 0 40px rgba(255,215,0,0.9), 0 4px 10px rgba(0,0,0,0.5)',
        opacity: phase >= 2 ? 1 : 0, transform: phase >= 2 ? 'translateY(0)' : 'translateY(20px)',
        transition:'all 0.5s ease-out 0.1s', textAlign:'center', lineHeight:1.1,
        position:'relative', zIndex:10,
      }}>ESPANHA</div>

      <div style={{
        fontSize:'clamp(11px,2.5vw,18px)', color:'rgba(255,215,0,0.75)',
        letterSpacing:'0.15em', textTransform:'uppercase',
        fontFamily:'system-ui', fontWeight:600, textAlign:'center', marginTop:6,
        opacity: phase >= 2 ? 1 : 0, transition:'opacity 0.5s ease-out 0.2s',
        position:'relative', zIndex:10,
      }}>Copa do Mundo 2026 · USA · Canada · México</div>

      <div style={{
        fontSize:'clamp(14px,3vw,22px)', color:'rgba(255,215,0,0.8)',
        fontFamily:'system-ui', fontWeight:700, textAlign:'center', marginTop:8,
        letterSpacing:'0.1em',
        opacity: phase >= 2 ? 1 : 0, transition:'opacity 0.5s ease-out 0.3s',
        position:'relative', zIndex:10,
      }}>🏆 2010 &nbsp;·&nbsp; 2026 🏆</div>

      {/* barra */}
      {phase >= 3 && (
        <div style={{width:'min(220px,60vw)',height:3,background:'rgba(255,255,255,0.2)',borderRadius:99,overflow:'hidden',marginTop:24,position:'relative',zIndex:10}}>
