...
function isLocked(jogo, override = {}) {
  // se tem override para esse jogo, usa o dataJogo real como limite (sem -1h)
  if (override[jogo.id]) {
    return Date.now() >= new Date(jogo.dataJogo).getTime();
  }
  // semifinais e final: bloquear só no início do jogo (sem -1h) para dar mais tempo
  if ([101, 102, 103, 104].includes(jogo.id)) {
    return Date.now() >= new Date(jogo.dataJogo).getTime();
  }
...
function calcPontosExtras(usuario, extras, resultadosFinais, todosExtras) {
  ...
  if (rf.artilheiro) {
    const artOficial = norm(rf.artilheiro.split(',')[0]);
    const palpite = norm(ex.artilheiro);
    if (palpite && palpite === artOficial) {
      const acertadores = todosExtras
        ? Object.values(todosExtras).filter(e => norm((e || {}).artilheiro) === artOficial).length
        : 1;
      pts += acertadores > 1 ? 25 : 50;
    }
  }
  return pts;
}
...
function MatchCard({ jogo, palpite, locked }) {
  ...
  const winA = finalizado && (jogo.placarOficialA > jogo.placarOficialB || (jogo.placarOficialA === jogo.placarOficialB && jogo.avancouPenaltis === 'A'));
  const winB = finalizado && (jogo.placarOficialB > jogo.placarOficialA || (jogo.placarOficialA === jogo.placarOficialB && jogo.avancouPenaltis === 'B'));
...
function SplashScreen({ onDone }) {
  const times = [
    { nome: 'França',     cc: 'fr' },
    { nome: 'Espanha',    cc: 'es' },
    { nome: 'Inglaterra', cc: 'gb-eng' },
    { nome: 'Argentina',  cc: 'ar' },
  ];

  const N = times.length;
  const STEP_BASE = 220;
  const FINAL_IDX = React.useRef(Math.floor(Math.random() * N)).current;

  const [current, setCurrent] = React.useState(0);
  const [isFinal, setIsFinal] = React.useState(false);
  const [showPergunta, setShowPergunta] = React.useState(false);
  const [barWidth, setBarWidth] = React.useState(0);
  const [hiding, setHiding] = React.useState(false);
  const [ringStop, setRingStop] = React.useState(false);

  function flagUrl(cc) { return `https://flagcdn.com/w160/${cc}.png`; }

  function gerarSequencia() {
    const seq = [];
    for (let i = 0; i < N * 3; i++) seq.push(i % N);
    for (let i = 0; i < N; i++) {
      const idx = i % N;
      seq.push(idx);
  ...
  function playSound(sequencia) {
    try {
      const ctx = new (window.AudioContext || window.webkitAudioContext)();
      sequencia.forEach((idx, step) => {
        const t = ctx.currentTime + step * (STEP_BASE / 1000);
        const o = ctx.createOscillator(), g = ctx.createGain();
        o.connect(g); g.connect(ctx.destination);
        o.type = step === sequencia.length - 1 ? 'triangle' : 'sine';
        o.frequency.value = step === sequencia.length - 1 ? 600 : 700 + Math.random() * 300;
        g.gain.setValueAtTime(step === sequencia.length - 1 ? 0.3 : 0.12, t);
        g.gain.exponentialRampToValueAtTime(0.001, t + (step === sequencia.length - 1 ? 0.4 : 0.1));
        o.start(t); o.stop(t + 0.4);
      });
      const tFinal = ctx.currentTime + sequencia.length * (STEP_BASE / 1000) + 0.4;
      ...
      cg.gain.setValueAtTime(0, tFinal); cg.gain.linearRampToValueAtTime(0.5, tFinal + 0.7); cg.gain.linearRampToValueAtTime(0, tFinal + 2.5);
      src.start(tFinal);
      const w = ctx.createOscillator(), wg = ctx.createGain();
      w.connect(wg); wg.connect(ctx.destination); w.type = 'sine';
      w.frequency.setValueAtTime(2800, tFinal + 0.2); w.frequency.linearRampToValueAtTime(3400, tFinal + 0.7);
      wg.gain.setValueAtTime(0.3, tFinal + 0.2); wg.gain.exponentialRampToValueAtTime(0.001, tFinal + 1);
      w.start(tFinal + 0.2); w.stop(tFinal + 1);
    } catch(e) {}
  }
  ...
  React.useEffect(() => {
    ...
    function avancar() {
      step++;
      if (step >= sequencia.length) {
        setRingStop(true); setIsFinal(true); setCurrent(FINAL_IDX);
        setTimeout(() => {
          setShowPergunta(true);
          let p = 0;
          const iv = setInterval(() => {
            p += 3; setBarWidth(p);
            if (p >= 100) { clearInterval(iv); setTimeout(() => { setHiding(true); setTimeout(onDone, 600); }, 300); }
          }, 30);
        }, 1000);
        return;
      }
      const restantes = sequencia.length - step;
      const delay = restantes <= 5 ? STEP_BASE + (5 - restantes) * 60 : STEP_BASE;
      setCurrent(sequencia[step]);
      setTimeout(avancar, delay);
    }
    setTimeout(avancar, STEP_BASE);
  }, []);

  const t = times[current];
  const colors = ['#FFD700','#FFFFFF','#003399','#75AADB','#C8102E','#FFFFFF'];

  return (
    <div style={{
      position:'fixed', inset:0, zIndex:999,
      background:'radial-gradient(ellipse at 50% 40%, #1530A6 0%, #0A1A5C 70%)',
      display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
      opacity: hiding ? 0 : 1, transition:'opacity 0.6s',
      fontFamily:"'Impact','Arial Black',system-ui,sans-serif", overflow:'hidden',
    }}>
      <style>{`
        @keyframes ringRotate{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}
        @keyframes finalPulse{0%,100%{box-shadow:0 0 0 4px #FFD700,0 0 60px rgba(255,215,0,1),0 0 100px rgba(255,215,0,0.6)}50%{box-shadow:0 0 0 5px #FFD700,0 0 80px rgba(255,215,0,1),0 0 140px rgba(255,215,0,0.8)}}
        @keyframes cfFall{0%{opacity:1;transform:translateY(0) rotate(0deg)}100%{opacity:0;transform:translateY(110vh) rotate(540deg)}}
        @keyframes perguntaIn{from{opacity:0;transform:translateY(15px)}to{opacity:1;transform:translateY(0)}}
        @keyframes ll{0%,100%{transform:rotate(6deg)}50%{transform:rotate(20deg)}}
        @keyframes lr{0%,100%{transform:rotate(-6deg)}50%{transform:rotate(-20deg)}}
      `}</style>

      <div style={{position:'absolute',top:0,left:'12%',width:2,height:'55vh',background:'linear-gradient(to bottom,rgba(255,220,100,0.3),transparent)',transformOrigin:'top center',animation:'ll 5s ease-in-out infinite'}}/>
      <div style={{position:'absolute',top:0,right:'12%',width:2,height:'55vh',background:'linear-gradient(to bottom,rgba(255,220,100,0.3),transparent)',transformOrigin:'top center',animation:'lr 5s ease-in-out infinite'}}/>

      {isFinal && colors.map((color, ci) =>
        Array.from({length:6}).map((_, i) => (
          <div key={`${ci}-${i}`} style={{
            position:'absolute', left:`${Math.random()*100}%`,
            width:`${6+Math.random()*9}px`, height:`${6+Math.random()*9}px`,
            background:color, borderRadius:2,
            animation:`cfFall ${2.5+Math.random()*3}s ${Math.random()*1.2}s linear infinite`,
          }}/>
        ))
      )}

      {/* título */}
      <div style={{position:'absolute',top:'7%',textAlign:'center',zIndex:10}}>
        <div style={{fontSize:'clamp(18px,4vw,34px)',color:'#FFD700',letterSpacing:'0.1em',textShadow:'0 0 30px rgba(255,215,0,0.7),0 3px 8px rgba(0,0,0,0.5)'}}>
          🏆 COPA DO MUNDO 2026 🏆
        </div>
        <div style={{fontSize:'clamp(11px,2.5vw,18px)',color:'rgba(255,255,255,0.8)',letterSpacing:'0.2em',fontFamily:'system-ui',fontWeight:700,marginTop:5,textTransform:'uppercase'}}>
          ⚽ Semifinais · 4 Seleções ⚽
        </div>
      </div>

      {/* anel — 4 bandeiras grandes */}
      <div style={{
        position:'relative', width:'min(340px,78vw)', height:'min(340px,78vw)',
        animation: ringStop ? 'none' : 'ringRotate 12s linear infinite', flexShrink:0,
      }}>
        {times.map((tm, i) => {
          const angle = (i / N) * 360;
          ...
          const isHL = i === current;
          const isFinalSlot = isFinal && i === FINAL_IDX;
          return (
            <div key={i} style={{
              position:'absolute', top:'50%', left:'50%',
              width:'min(100px,22vw)', height:'min(100px,22vw)',
              marginTop:'calc(min(100px,22vw) / -2)', marginLeft:'calc(min(100px,22vw) / -2)',
              transform:`translate(${x}px, ${y}px)`,
            }}>
              <div style={{
                width:'100%', height:'100%', borderRadius:'50%', overflow:'hidden', position:'relative',
                border: isHL ? '4px solid #FFD700' : '3px solid rgba(255,255,255,0.2)',
                boxShadow: isFinalSlot ? '0 0 0 4px #FFD700,0 0 60px rgba(255,215,0,1),0 0 100px rgba(255,215,0,0.6)'
                  : isHL ? '0 0 0 3px #FFD700,0 0 30px rgba(255,215,0,0.9)' : '0 4px 20px rgba(0,0,0,0.5)',
                transform: isHL ? 'scale(1.5)' : 'scale(1)',
                transition:'all 0.2s ease',
                animation: isFinalSlot ? 'finalPulse 0.7s ease-in-out infinite' : 'none',
                zIndex: isHL ? 20 : 1,
              }}>
                <img src={flagUrl(tm.cc)} alt={tm.nome} style={{width:'140%',height:'140%',objectFit:'cover',position:'absolute',top:'-20%',left:'-20%'}}/>
              </div>
            </div>
          );
        })}
      ...
      <div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',textAlign:'center',zIndex:5,pointerEvents:'none'}}>
        <div style={{
          width:'min(120px,26vw)', height:'min(120px,26vw)', borderRadius:'50%', overflow:'hidden',
          border:'4px solid #FFD700',
          boxShadow: isFinal ? '0 0 0 4px #FFD700,0 0 60px rgba(255,215,0,0.9),0 0 100px rgba(255,215,0,0.6)' : '0 0 0 3px rgba(255,215,0,0.4),0 0 40px rgba(255,215,0,0.5)',
          margin:'0 auto 8px', transition:'box-shadow 0.3s',
          animation: isFinal ? 'finalPulse 0.7s ease-in-out infinite' : 'none',
        }}>
          <img src={flagUrl(t.cc)} alt={t.nome} style={{width:'140%',height:'140%',objectFit:'cover',marginLeft:'-20%',marginTop:'-20%'}}/>
        </div>
        <div style={{fontSize: isFinal ? 'clamp(16px,3.5vw,28px)' : 'clamp(13px,3vw,22px)', color:'#FFD700', textShadow:'0 0 20px rgba(255,215,0,0.8)', letterSpacing:'0.06em', transition:'all 0.3s'}}>
          {t.nome}
        </div>
      </div>
      ...
      {/* pergunta */}
      {showPergunta && (
        <div style={{position:'absolute',bottom:'8%',textAlign:'center',zIndex:10,animation:'perguntaIn 0.8s ease-out forwards'}}>
          <p style={{fontSize:'clamp(14px,3vw,24px)',color:'#fff',letterSpacing:'0.06em',textShadow:'0 2px 8px rgba(0,0,0,0.5)'}}>Quem será o</p>
          <span style={{color:'#FFD700',fontSize:'clamp(18px,4vw,34px)',textShadow:'0 0 30px rgba(255,215,0,0.8)',display:'block',marginTop:5}}>⚽ CAMPEÃO? ⚽</span>
          <div style={{width:'min(200px,55vw)',height:3,background:'rgba(255,255,255,0.15)',borderRadius:99,overflow:'hidden',margin:'14px auto 0'}}>
            <div style={{height:'100%',width:`${barWidth}%`,background:'#FFD700',borderRadius:99,transition:'width 0.03s linear'}}/>
          </div>
        </div>
      )}
    </div>
...
export default function BolaoApp() {
  ...
  const [pendingPenaltis, setPendingPenaltis] = useState({}); // jogoId -> 'A' | 'B'
  ...
  const [jogosBloqueioOverride, setJogosBloqueioOverride] = useState({}); // jogoId -> timestamp limite
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        const confrontosFixos = {
          ...
          97: { 'timeA': 'França', 'ccA': 'fr', 'timeB': 'Marrocos', 'ccB': 'ma', 'dataJogo': '2026-07-09T17:00:00', 'cidade': 'Boston' },
          98: { 'timeA': 'Espanha', 'ccA': 'es', 'timeB': 'Bélgica', 'ccB': 'be', 'dataJogo': '2026-07-10T16:00:00', 'cidade': 'Los Angeles' },
          99: { 'timeA': 'Noruega', 'ccA': 'no', 'timeB': 'Inglaterra', 'ccB': 'gb-eng', 'dataJogo': '2026-07-11T18:00:00', 'cidade': 'Miami' },
          100: { 'timeA': 'Argentina', 'ccA': 'ar', 'timeB': 'Suíça', 'ccB': 'ch', 'dataJogo': '2026-07-11T22:00:00', 'cidade': 'Kansas City' },
          101: { 'timeA': 'França', 'ccA': 'fr', 'timeB': 'Espanha', 'ccB': 'es', 'dataJogo': '2026-07-14T16:00:00', 'cidade': 'Dallas' },
          102: { 'timeA': 'Inglaterra', 'ccA': 'gb-eng', 'timeB': 'Argentina', 'ccB': 'ar', 'dataJogo': '2026-07-15T16:00:00', 'cidade': 'Atlanta' },
          103: { 'timeA': 'França', 'ccA': 'fr', 'timeB': 'Inglaterra', 'ccB': 'gb-eng', 'dataJogo': '2026-07-18T16:00:00', 'cidade': 'Miami' },
          104: { 'timeA': 'Espanha', 'ccA': 'es', 'timeB': 'Argentina', 'ccB': 'ar', 'dataJogo': '2026-07-19T16:00:00', 'cidade': 'Nova York/NJ' }
        };
        if (data.jogos) {
          let corrigiu = false;
  ...
  useEffect(() => {
    ...
    function nomeVencedor(jogo) {
      ...
      if (jogo.avancouPenaltis === 'A') return jogo.timeA;
      if (jogo.avancouPenaltis === 'B') return jogo.timeB;
      return null; // empate sem pênaltis definido ainda
    }
    function nomePerdedor(jogo) {
      ...
      // empate — verificar pênaltis
      if (jogo.avancouPenaltis === 'A') return jogo.timeB;
      if (jogo.avancouPenaltis === 'B') return jogo.timeA;
      return null;
    }
  ...
  const toggleOverrideBloqueio = (jogoId) => {
    setJogosBloqueioOverride(prev => {
      const novo = { ...prev };
      if (novo[jogoId]) delete novo[jogoId];
      else novo[jogoId] = true;
      return novo;
    });
    const jogo = state.jogos.find(j => j.id === +jogoId);
    const ativando = !jogosBloqueioOverride[jogoId];
    setSaveMsg(ativando
      ? `✅ Jogo ${jogoId} desbloqueado até o início (${new Date(jogo?.dataJogo).toLocaleString('pt-BR', {hour:'2-digit',minute:'2-digit'})})`
      : `🔒 Jogo ${jogoId} voltou ao bloqueio normal`
    );
    setTimeout(() => setSaveMsg(''), 3000);
  };

  const corrigirPenaltisJogo = (jogoId, quemAvancou) => {
    const novosJogos = state.jogos.map(j =>
      j.id === jogoId ? { ...j, avancouPenaltis: quemAvancou } : j
    );
    persist({ ...state, jogos: novosJogos });
    setSaveMsg(`Pênaltis do Jogo ${jogoId} registrado — avançou: ${quemAvancou === 'A' ? state.jogos.find(j=>j.id===jogoId)?.timeA : state.jogos.find(j=>j.id===jogoId)?.timeB}!`);
    setTimeout(() => setSaveMsg(''), 3000);
  };
  ...
  const atualizarPalpitesClaude = () => {
    ...
    // remover TODOS os palpites existentes do Claude no banco
    const semClaude = Object.fromEntries(
      Object.entries(state.palpites).filter(([k]) => !k.endsWith('_Claude'))
    );
    // adicionar apenas os palpites legítimos (a partir de 18/06)
    const novosPalpites = { ...semClaude, ...PALPITES_CLAUDE };
    persist({ ...state, usuarios, palpites: novosPalpites });
    setSaveMsg(`Palpites do Claude atualizados — ${Object.keys(PALPITES_CLAUDE).length} válidos, anteriores removidos!`);
    setTimeout(() => setSaveMsg(''), 3000);
  };
  ...
  const gerarAvisoWhatsapp = () => {
    ...
    const jogosProximos = state.jogos
      .filter(j => {
        ...
        return d >= inicioHoje && d < fimAmanha && !isLocked(j, jogosBloqueioOverride);
      })
  ...
  const salvarPalpite = (jogoId) => {
    ...
    const jogo = state.jogos.find(j => j.id === jogoId);
    if (isLocked(jogo, jogosBloqueioOverride)) return;
  ...
  const salvarResultado = (jogoId) => {
    ...
    if (!pend || pend.a === '' || pend.b === '') return;
    const pen = pendingPenaltis[jogoId]; // 'A', 'B' ou undefined
    const novosJogos = state.jogos.map(j => j.id === jogoId ? {
      ...j,
      placarOficialA: pend.a,
      placarOficialB: pend.b,
      avancouPenaltis: (pend.a === pend.b && pen) ? pen : null,
    } : j);
    persist({ ...state, jogos: novosJogos });
  ...
  // EXCETO jogos eliminatórios empatados sem avancouPenaltis definido
  const limite24h = Date.now() - 24 * 60 * 60 * 1000;
  jogosAdmin = jogosAdmin.filter(j => {
    const finalizado = j.placarOficialA !== null && j.placarOficialA !== undefined;
    if (!finalizado) return true; // sem resultado sempre aparece
    const empateEliminatório = j.fase !== 'grupos' && j.placarOficialA === j.placarOficialB && !j.avancouPenaltis;
    if (empateEliminatório) return true; // empate sem pênaltis sempre aparece
    const inicio = new Date(j.dataJogo).getTime();
    return inicio > limite24h;
  });

  const ranking = state.usuarios.map(u => {
    let ptsJogos = 0, exatos = 0, vencedores = 0, errados = 0, faltando = 0;
    state.jogos.forEach(jogo => {
      ...
      if (finalizado) {
        if (!palpite) faltando++;
          ...
          if (pts === faseInfo.exato) exatos++;
          else if (pts === faseInfo.vencedor) vencedores++;
          else errados++;
        }
      }
    });
    const ptsExtras = calcPontosExtras(u, state.extras, state.resultadosFinais, state.extras);
    return { usuario: u, ptsJogos, ptsExtras, total: ptsJogos + ptsExtras, exatos, vencedores, errados, faltando };
    ...
    <div style={page}>
      ...
      <div style={{ ...container, position: 'relative', zIndex: 1 }}>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {proximosResumo.map(r => {
                const bloqueado = isLocked(r.jogo, jogosBloqueioOverride);
                return (
                <div key={r.jogo.id}>
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {jogosFiltrados.map(jogo => {
                  ...
                  const locked = isLocked(jogo, jogosBloqueioOverride);
                  ...
                  return (
                    <div key={jogo.id} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        {jogo.avancouPenaltis && (
                          <Badge tone="default">🥅 {jogo.avancouPenaltis === 'A' ? jogo.timeA : jogo.timeB} nos pên.</Badge>
                        )}
                      </div>
                    </div>
                  );
        ...
        {effectiveTab === 'extras' && (
          ...
          <div style={card}>
            <div style={h3}>Palpites extras de todos</div>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ background: '#EEF2FF' }}>
                    <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>Participante</th>
                    <th style={{ padding: '8px 10px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>🥇 Campeão</th>
                    <th style={{ padding: '8px 10px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>🥈 Vice</th>
                    <th style={{ padding: '8px 10px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>🥉 3º Lugar</th>
                    <th style={{ padding: '8px 10px', textAlign: 'center', fontWeight: 700, color: '#475569', fontSize: 11, textTransform: 'uppercase' }}>👟 Artilheiro</th>
                  </tr>
                </thead>
                <tbody>
                  {state.usuarios.map((u) => {
                    const ex = state.extras[u] || {};
                    const rf = state.resultadosFinais || {};
                    const norm = (s) => (s || '').trim().toLowerCase();
                    const acertouCampeao = rf.campeao && norm(ex.campeao) === norm(rf.campeao);
                    const acertouVice = rf.vice && norm(ex.vice) === norm(rf.vice);
                    const acertouTerceiro = rf.terceiro && norm(ex.terceiro) === norm(rf.terceiro);
                    const acertouArtilheiro = rf.artilheiro && rf.artilheiro.split(',').map(norm).includes(norm(ex.artilheiro));
                    return (
                      <tr key={u} style={{ borderTop: '1px solid #F1F5F9', background: u === usuario ? '#EEF2FF' : 'transparent' }}>
                        <td style={{ padding: '8px 10px', fontWeight: 700 }}>{u}</td>
                        {[
                          { val: ex.campeao, acertou: acertouCampeao },
                          { val: ex.vice, acertou: acertouVice },
                          { val: ex.terceiro, acertou: acertouTerceiro },
                          { val: ex.artilheiro, acertou: acertouArtilheiro },
                        ].map((item, j) => (
                          <td key={j} style={{ padding: '8px 10px', textAlign: 'center' }}>
                            {item.val ? (
                              <span style={{
                                fontWeight: 600, fontSize: 13,
                                color: item.acertou ? '#16A34A' : '#475569',
                                background: item.acertou ? '#F0FDF4' : 'transparent',
                                padding: item.acertou ? '2px 8px' : 0,
                                borderRadius: 6,
                              }}>
                                {item.acertou ? '✅ ' : ''}{item.val}
                              </span>
                            ) : (
        ...
        {effectiveTab === 'admin' && (
          <>
            ...
            <div style={card}>
              <div style={h3}>🔓 Liberar palpite de jogo específico</div>
              <div style={{ fontSize: 13, color: '#64748B', marginBottom: 14 }}>
                Libera temporariamente o palpite de um jogo já bloqueado (1h antes), permitindo palpites até o início real do jogo. O desbloqueio é apenas nesta sessão.
              </div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                {state.jogos
                  .filter(j => {
                    const limite = new Date(j.dataJogo).getTime() - 60 * 60 * 1000;
                    const inicio = new Date(j.dataJogo).getTime();
                    return Date.now() >= limite && Date.now() < inicio;
                  })
                  .sort((a, b) => new Date(a.dataJogo) - new Date(b.dataJogo))
                  .map(j => {
                    const ativo = jogosBloqueioOverride[j.id];
                    return (
                      <button key={j.id} onClick={() => toggleOverrideBloqueio(j.id)} style={{
                        padding: '8px 14px', borderRadius: 8, border: 'none', cursor: 'pointer',
                        fontWeight: 700, fontSize: 13,
                        background: ativo ? '#16A34A' : '#FEF2F2',
                        color: ativo ? '#fff' : '#B91C1C',
                      }}>
                        {ativo ? '✅' : '🔒'} {j.timeA} x {j.timeB}
                      </button>
                    );
                  })}
                {state.jogos.filter(j => {
                  const limite = new Date(j.dataJogo).getTime() - 60 * 60 * 1000;
                  const inicio = new Date(j.dataJogo).getTime();
                  return Date.now() >= limite && Date.now() < inicio;
                }).length === 0 && (
                  <div style={{ fontSize: 13, color: '#94A3B8', fontStyle: 'italic' }}>
                    Nenhum jogo no período de bloqueio (1h antes do início) agora.
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={card}>
              <div style={h3}>🥅 Registrar pênaltis em jogo encerrado</div>
              <div style={{ fontSize: 13, color: '#64748B', marginBottom: 14 }}>
                Use para jogos eliminatórios que terminaram empatados e já saíram da lista (ex: Austrália x Egito). Selecione o jogo e indique quem avançou nos pênaltis.
              </div>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                <select style={select} onChange={e => {
                  const jid = +e.target.value;
                  const j = state.jogos.find(x => x.id === jid);
                  if (!j) return;
                  // mostrar botões de seleção
                  document.getElementById('pen-time-a').textContent = j.timeA;
                  document.getElementById('pen-time-b').textContent = j.timeB;
                  document.getElementById('pen-jogo-id').value = jid;
                  document.getElementById('pen-btns').style.display = 'flex';
                }}>
                  <option value="">Selecione o jogo...</option>
                  {state.jogos
                    .filter(j => j.fase !== 'grupos' && j.placarOficialA !== null && j.placarOficialA !== undefined)
                    .sort((a, b) => a.id - b.id)
                    .map(j => (
                      <option key={j.id} value={j.id}>
                        Jogo {j.id} — {j.timeA} {j.placarOficialA}x{j.placarOficialB} {j.timeB}
                        {j.avancouPenaltis ? ` ✓ ${j.avancouPenaltis === 'A' ? j.timeA : j.timeB}` : ''}
                      </option>
                    ))}
                </select>
              </div>
              <input type="hidden" id="pen-jogo-id" value="" />
              <div id="pen-btns" style={{ display: 'none', gap: 10, alignItems: 'center', marginTop: 12, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#7C3AED' }}>Quem avançou?</span>
                <PrimaryButton small onClick={() => {
                  const jid = +document.getElementById('pen-jogo-id').value;
                  if (jid) corrigirPenaltisJogo(jid, 'A');
                }}>
                  <span id="pen-time-a">Time A</span>
                </PrimaryButton>
                <PrimaryButton small onClick={() => {
                  const jid = +document.getElementById('pen-jogo-id').value;
                  if (jid) corrigirPenaltisJogo(jid, 'B');
                }}>
                  <span id="pen-time-b">Time B</span>
                </PrimaryButton>
              </div>
            </div>
                  ...
                  return (
                    <div key={jogo.id} style={{
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' }}>
                        ...
                        {/* seletor de pênaltis — aparece só em empate em fase eliminatória */}
                        {jogo.fase !== 'grupos' && valA !== '' && valB !== '' && +valA === +valB && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                            <span style={{ color: '#7C3AED', fontWeight: 700 }}>🥅 Pênaltis — avançou:</span>
                            <button
                              onClick={() => setPendingPenaltis(s => ({ ...s, [jogo.id]: s[jogo.id] === 'A' ? null : 'A' }))}
                              style={{ padding: '3px 10px', borderRadius: 6, border: 'none', fontWeight: 700, fontSize: 12, cursor: 'pointer', background: pendingPenaltis[jogo.id] === 'A' ? '#7C3AED' : '#EDE9FE', color: pendingPenaltis[jogo.id] === 'A' ? '#fff' : '#7C3AED' }}>
                              {jogo.timeA}
                            </button>
                            <button
                              onClick={() => setPendingPenaltis(s => ({ ...s, [jogo.id]: s[jogo.id] === 'B' ? null : 'B' }))}
                              style={{ padding: '3px 10px', borderRadius: 6, border: 'none', fontWeight: 700, fontSize: 12, cursor: 'pointer', background: pendingPenaltis[jogo.id] === 'B' ? '#7C3AED' : '#EDE9FE', color: pendingPenaltis[jogo.id] === 'B' ? '#fff' : '#7C3AED' }}>
                              {jogo.timeB}
                            </button>
                          </div>
                        )}
                        {/* mostrar pênaltis já gravado */}
                        {jogo.avancouPenaltis && (
                          <div style={{ fontSize: 11, color: '#7C3AED', fontWeight: 600 }}>
                            🥅 Avançou nos pênaltis: {jogo.avancouPenaltis === 'A' ? jogo.timeA : jogo.timeB}
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {effectiveTab === 'rank' && (
          <div style={card}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ textAlign: 'left', color: '#64748B', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    <th style={{ padding: '8px 8px' }}>Pos.</th>
                    <th style={{ padding: '8px 8px' }}>Participante</th>
                    <th style={{ padding: '8px 8px', textAlign: 'center' }}>🎯 Exatos</th>
                    <th style={{ padding: '8px 8px', textAlign: 'center' }}>✅ Vencedor</th>
                    <th style={{ padding: '8px 8px', textAlign: 'center' }}>❌ Errados</th>
                    <th style={{ padding: '8px 8px', textAlign: 'center' }}>⬜ Faltando</th>
                    <th style={{ padding: '8px 8px', textAlign: 'right' }}>Pts Jogos</th>
                    <th style={{ padding: '8px 8px', textAlign: 'right' }}>Pts Extras</th>
                    <th style={{ padding: '8px 8px', textAlign: 'right' }}>Total</th>
                  </tr>
                </thead>
                <tbody>
                  {ranking.map((r, i) => (
                    <tr key={r.usuario} style={{ borderTop: '1px solid #F1F5F9', background: r.usuario === usuario ? '#EEF2FF' : 'transparent' }}>
                      <td style={{ padding: '8px 8px', fontWeight: 800, color: i < 3 ? '#0E2080' : '#94A3B8' }}>
                        {i + 1}º{i === 0 ? ' 🏆' : ''}
                      </td>
                      <td style={{ padding: '8px 8px', fontWeight: 700 }}>{r.usuario}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'center', fontWeight: 700, color: '#16A34A' }}>{r.exatos}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'center', fontWeight: 700, color: '#2563EB' }}>{r.vencedores}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'center', fontWeight: 700, color: '#DC2626' }}>{r.errados}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'center', fontWeight: 700, color: '#94A3B8' }}>{r.faltando}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'right', color: '#475569' }}>{r.ptsJogos}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'right', color: '#475569' }}>{r.ptsExtras}</td>
                      <td style={{ padding: '8px 8px', textAlign: 'right', fontWeight: 800, fontSize: 15 }}>{r.total}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
