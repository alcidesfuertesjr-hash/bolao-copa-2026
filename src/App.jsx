...
// calcula nomes reais para os placeholders da Rodada de 32, ou null se ainda não dá pra definir
function calcularClassificadosR32(jogos) {
  const agora = Date.now();
  const gruposProntos = {};
  GRUPOS_LETRAS.forEach(g => { gruposProntos[g] = grupoConcluido(jogos, g, agora); });

  const todosGruposProntos = GRUPOS_LETRAS.every(g => gruposProntos[g]);
  const classificacao = calcularClassificacaoGrupos(jogos);

  // melhores terceiros: só calculável quando TODOS os grupos estiverem prontos
  let melhoresTerceiros = null;
  if (todosGruposProntos) {
    const terceiros = GRUPOS_LETRAS.map(g => ({ grupo: g, ...classificacao[g][2] }))
      .filter(t => t.time)
      .sort((x, y) => y.pts - x.pts || y.sg - x.sg || y.gp - x.gp);
    melhoresTerceiros = terceiros.slice(0, 8);
  }

  const resolverSlot = (slot) => {
    if (slot.tipo === 'pos') {
      if (!gruposProntos[slot.grupo]) return null;
      const time = classificacao[slot.grupo][slot.posicao - 1];
      return time ? time.time : null;
    }
    if (slot.tipo === 'terceiro') {
      if (!melhoresTerceiros) return null;
      const achou = melhoresTerceiros.find(t => slot.grupos.includes(t.grupo));
      return achou ? achou.time : null;
    }
    return null;
  };

  const atualizacoes = {};
  Object.entries(CRUZAMENTO_R32).forEach(([jogoId, cruzamento]) => {
    const nomeA = resolverSlot(cruzamento.a);
    const nomeB = resolverSlot(cruzamento.b);
    if (nomeA || nomeB) atualizacoes[jogoId] = { nomeA, nomeB };
  });
  return atualizacoes;
}
...
function BracketView({ jogos, fase, palpites, usuario, usuarios }) {
  const jogosF = jogos.filter(j => j.fase === fase).sort((a, b) => new Date(a.dataJogo) - new Date(b.dataJogo));

  if (jogosF.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '32px 0', color: '#94A3B8', fontSize: 13.5 }}>
        Jogos desta fase ainda não disponíveis — serão preenchidos automaticamente conforme as fases anteriores forem concluídas.
      </div>
    );
  }

  const isPlaceholder = (nome) => nome && (nome.startsWith('1º') || nome.startsWith('2º') || nome.startsWith('Melhor'));

  return (
    <div style={{ overflowX: 'auto', paddingBottom: 8 }}>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.ceil(jogosF.length / 2)}, minmax(200px, 1fr))`, gap: 10, minWidth: jogosF.length > 4 ? 700 : 'auto' }}>
        {jogosF.map(jogo => {
          const key = `${jogo.id}_${usuario}`;
          const palpite = palpites[key];
          const finalizado = jogo.placarOficialA !== null && jogo.placarOficialA !== undefined;
          const placeholderA = isPlaceholder(jogo.timeA);
          const placeholderB = isPlaceholder(jogo.timeB);
          ...
          return (
            <div key={jogo.id} style={{
              ...
              boxShadow: '0 1px 4px rgba(0,0,0,0.08)', border: '1px solid #EEF1F5',
            }}>
              {/* header */}
              <div style={{ background: '#0E2080', padding: '6px 10px', fontSize: 11, color: '#FFD600', fontWeight: 700, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div style={{
                ...
                background: finalizado && jogo.placarOficialA > jogo.placarOficialB ? '#F0FDF4' : '#fff',
                borderBottom: '1px solid #F1F5F9',
              }}>
                {!placeholderA && <Flag cc={jogo.ccA} nome={jogo.timeA} />}
                <span style={{ fontWeight: placeholderA ? 500 : 700, fontSize: 13, color: placeholderA ? '#94A3B8' : '#0F172A', fontStyle: placeholderA ? 'italic' : 'normal', flex: 1 }}>
                  {jogo.timeA}
                </span>
                {finalizado && (
                  <span style={{ fontWeight: 800, fontSize: 18, color: jogo.placarOficialA > jogo.placarOficialB ? '#16A34A' : '#64748B' }}>
                    {jogo.placarOficialA}
                  </span>
                )}
              </div>
              ...
              <div style={{
                display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px',
                background: finalizado && jogo.placarOficialB > jogo.placarOficialA ? '#F0FDF4' : '#fff',
              }}>
                {!placeholderB && <Flag cc={jogo.ccB} nome={jogo.timeB} />}
                <span style={{ fontWeight: placeholderB ? 500 : 700, fontSize: 13, color: placeholderB ? '#94A3B8' : '#0F172A', fontStyle: placeholderB ? 'italic' : 'normal', flex: 1 }}>
                  {jogo.timeB}
                </span>
                {finalizado && (
                  <span style={{ fontWeight: 800, fontSize: 18, color: jogo.placarOficialB > jogo.placarOficialA ? '#16A34A' : '#64748B' }}>
                    {jogo.placarOficialB}
                  </span>
                )}
              </div>
              ...
              {/* palpite do usuário */}
              <div style={{ padding: '6px 12px', background: '#F8FAFC', borderTop: '1px solid #F1F5F9', fontSize: 11.5, color: '#64748B', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>Seu palpite:</span>
                <span style={{ fontWeight: 700, color: '#0E2080' }}>
...
export default function BolaoApp() {
  ...
  const [tooltip, setTooltip] = useState(null); // { jogoId, tipo: 'A'|'empate'|'B', nomes: [] }
  ...
  // auto-atualização: substitui placeholders da Rodada de 32 pelos nomes reais
  // dos classificados, conforme os grupos forem sendo concluídos (placar lançado há 4h+)
  useEffect(() => {
    if (!state || !state.jogos) return;

    // 1) atualizar Rodada de 32 com classificados dos grupos
    const atualizacoesR32 = calcularClassificadosR32(state.jogos);

    // 2) propagar vencedores/perdedores de jogos já finalizados há 4h+
    const agora = Date.now();
    const mapaJogos = {};
    ...
    function nomeVencedor(jogo) {
      ...
      return null; // empate (na eliminatória isso não deve acontecer)
    }
    ...
    const novosJogos = state.jogos.map(j => {
      // atualização da R32 (grupos → dezesseis avos)
      const updR32 = atualizacoesR32[j.id];

      // atualização por propagação de vencedores
      let novoA = j.timeA;
      let novoB = j.timeB;

      // substituir "Vencedor Jogo X" pelo nome real
      const matchA = j.timeA && j.timeA.match(/^Vencedor Jogo (\d+)$/);
      const matchB = j.timeB && j.timeB.match(/^Vencedor Jogo (\d+)$/);
      const matchPA = j.timeA && j.timeA.match(/^Perdedor Jogo (\d+)$/);
      const matchPB = j.timeB && j.timeB.match(/^Perdedor Jogo (\d+)$/);

      if (matchA) { const v = nomeVencedor(mapaJogos[+matchA[1]]); if (v) novoA = v; }
      if (matchB) { const v = nomeVencedor(mapaJogos[+matchB[1]]); if (v) novoB = v; }
      if (matchPA) { const v = nomePerdedor(mapaJogos[+matchPA[1]]); if (v) novoA = v; }
      if (matchPB) { const v = nomePerdedor(mapaJogos[+matchPB[1]]); if (v) novoB = v; }

      // R32 tem prioridade sobre propagação simples (mais precisa)
      if (updR32) {
        if (updR32.nomeA) novoA = updR32.nomeA;
        if (updR32.nomeB) novoB = updR32.nomeB;
      }
      ...
      if (novoA !== j.timeA || novoB !== j.timeB) {
        ...
        return {
          ...
          timeA: novoA,
          timeB: novoB,
          ccA: NOME_PARA_CC[novoA] || j.ccA,
          ccB: NOME_PARA_CC[novoB] || j.ccB,
        };
      }
      return j;
    });
          ...
          <>
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {/* BRACKET para fases eliminatórias */}
                {['dezesseis','oitavas','quartas','semi','terceiro','final'].includes(filtroFase) && !mostrarProximos ? (
                  <BracketView
                    jogos={state.jogos}
                    fase={filtroFase}
