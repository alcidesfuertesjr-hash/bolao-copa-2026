...
export default function BolaoApp() {
  ...
  const CORRECOES_DATAS = {
    5: '2026-06-14T01:00:00',
    17: '2026-06-16T22:00:00',
    29: '2026-06-20T00:00:00',
  };

  const corrigirDatasJogos = () => {
    const novosJogos = state.jogos.map(j =>
      CORRECOES_DATAS[j.id] ? { ...j, dataJogo: CORRECOES_DATAS[j.id] } : j
    );
    persist({ ...state, jogos: novosJogos });
    setSaveMsg('Datas dos jogos 5, 17 e 29 corrigidas!');
    setTimeout(() => setSaveMsg(''), 3000);
  };
      ...
      <div style={{ ...container, position: 'relative', zIndex: 1 }}>
        ...
        {effectiveTab === 'admin' && (
          <>
            ...
            <div style={card}>
              <div style={h3}>Corrigir datas/horários de jogos</div>
              <div style={{ fontSize: 13, color: '#64748B', marginBottom: 14 }}>
                Corrige no banco de dados as datas dos jogos 5 (Austrália x Turquia → 14/06 01h),
                17 (Argentina x Argélia → 16/06 22h) e 29 (Turquia x Paraguai → 20/06 00h).
                Necessário apenas uma vez.
              </div>
              <PrimaryButton onClick={corrigirDatasJogos}>Corrigir datas dos jogos</PrimaryButton>
            </div>
